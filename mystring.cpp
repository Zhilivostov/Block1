//#pragma warning(disable : 4996) // êîñòûëü äëÿ strcpy è strncpy
//#pragma warning(disable : 6386) // êîñòûëü äëÿ 33 ñòðîêè (m_arr[n] = '\0')

#include "mystring.h"
#include <algorithm> //ïîäêëþ÷àåò std::min
#include <cstring> //ïîäêëþ÷àåò strlen - ñ÷èòàåò êîëè÷åñòâî ñèìâîëîâ äî äåòåðìåíèðîâàííîãî íóëÿ, strcpy - êîïèðóåò ñîäåðæèìîå âòîðîé ñòðîêè â ïåðâóþ, strncpy
#include <stdexcept>  //èñêëþ÷åíèÿ

void String::release()//ïðîâååðêà íà íóëïòð
{
...
void String::branch(size_t new_capacity)//ïðîâååðêà íà íóëïòð
{
	if (_countref == nullptr || _arr == nullptr)
	{
		throw std::invalid_argument("Passing a null pointer!");
...
void String::reserve(size_t n) //!!!!!!íå ëîìàåòñÿ ëè çäåñü count referense?
{
	if (n <= _capacity)
	{
		return;
...
{
	return strcmp(_arr, str.data()); //ñðàâíèâàåò ïîñèìâîëüíî ñòðîêè
}
